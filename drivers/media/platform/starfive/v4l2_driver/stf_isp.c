...
{
	if (which == V4L2_SUBDEV_FORMAT_TRY)
		return v4l2_subdev_state_get_compose(state,
						   STF_ISP_PAD_SINK);
...
{
	if (which == V4L2_SUBDEV_FORMAT_TRY)
		return v4l2_subdev_state_get_crop(state, STF_ISP_PAD_SINK);
		...
		struct v4l2_subdev_selection *sel)
{
	...
	if (sel->which == V4L2_SUBDEV_FORMAT_TRY)
		return v4l2_subdev_state_get_compose(state, STF_ISP_PAD_SINK);
